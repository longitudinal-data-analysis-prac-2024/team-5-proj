...
#Conduct disorder - classify people with or without conduct disorder based on the standardised scores provided in the cdspa variable. People that have 0 are classified as "no" (CD), people that have 1 or more are classified as "yes" (CD)."yes" - 1, "no" - 0
...
data_gathered<- data_CD %>% gather(Item, Score, ICU03:ICU22) %>% mutate(Score = recode(Score, 'not at all true' = '0', 'somewhat true' = '1', 'very true' = '2', 'definitely true' = '3'))%>% mutate(Score = as.numeric(Score))
...
ggplot(summary_Time_CD, aes(x = Time, y = mean, group = CD)) +
...
```{r}
#OR - much greater variability in the CD yes group
...
summary_Time_Grouping_CD <-  data_grouped %>% group_by(Time, Grouping, CD) %>%
  summarise(n=n(),
            ...
            lower=mean-se*1.96,
            upper=mean+se*1.96)
summary_Time_Grouping_CD

ggplot(summary_Time_Grouping_CD, aes(x = Time, y = mean, group = Grouping)) +
...
contrasts(data_CD_yes$Time) <- contr.poly(7) #polynomial contrast codes 
