...
data_summed<- data_CU_filtered %>% group_by(ID) %>% mutate(sum_score = sum(Score))
```
...
contrasts(data_grouped$Grouping) <- cbind(c1 =  c(2/3, -1/3, -1/3),
                                 c2 = c(-1/2, 0, 1/2))
contrasts(data_grouped$Grouping)

mod <- afex::mixed(Saliva ~ Time*Grouping + (1|ID), data=data_grouped, check_contrasts = FALSE) #andrea suggested to add random slopes of time to see how slopes vary.
summary(mod)
...
mod_simple<- afex::mixed(Saliva ~ 1 + Time + (Time|ID), data=data_grouped, check_contrasts = FALSE)
summary(mod_simple)
